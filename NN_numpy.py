...
class Layer:
    ...
    def __init__(self, size, previousSize):
        ...
        self.bias = np.array(
            [random.uniform(-0.3, 0.3) for _ in range(size)], dtype="float64"
        ...
        self.weights = np.array(
            [
                np.array(
                    [
                        random.uniform(
                            -1.0 / np.sqrt(previousSize), 1.0 / np.sqrt(previousSize)
                        )
        ...
        )  # pour chaque neurone, la liste des poids des arcs entrants (provenant de la couche précédente)
...
class NeuralNet:
    ...
    def __init__(self, layerSizes):
        self.layers = [
            Layer(layerSizes[i], layerSizes[i - 1] if i >= 1 else 0)
    ...
    def load(self, filename):
        with open(filename, "r") as file:
            ...
            for layer in self.layers[1:]:
                layer.bias = np.array(
                    [float(size) for size in lines[idx][:-1].split(" ")]
                )
                idx += 1
                for i in range(layer.size):
                    layer.weights[i] = np.array(
                        [float(size) for size in lines[idx][:-1].split(" ")]
                    )
                    idx += 1
